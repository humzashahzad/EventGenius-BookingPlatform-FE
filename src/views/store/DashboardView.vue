    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-accent-500);">
          ...
          <div><div class="kpi-value">{{ data.stats?.total_venues }}</div><div class="stat-label">Venues</div></div>
        </div>
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-success-500);">
          ...
          <div><div class="kpi-value text-success-600">{{ data.stats?.active_venues }}</div><div class="stat-label">Active</div></div>
        </div>
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-primary-500);">
          ...
          <div><div class="kpi-value">{{ data.stats?.total_bookings }}</div><div class="stat-label">Bookings</div></div>
        </div>
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-warning-500);">
          ...
          <div><div class="kpi-value text-warning-600">{{ data.stats?.pending_bookings }}</div><div class="stat-label">Pending</div></div>
        </div>
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-info-500);">
          <div class="stat-icon bg-info-100">
            <svg class="w-5 h-5 text-info-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
          </div>
          <div><div class="kpi-value text-info-600">{{ data.stats?.confirmed_bookings }}</div><div class="stat-label">Confirmed</div></div>
        </div>
        <div class="stat-card stat-card-accent" style="--accent-color: var(--color-success-500);">
          <div class="stat-icon bg-success-100">
            <svg class="w-5 h-5 text-success-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <div><div class="kpi-value text-success-600 !text-base">{{ formatRevenue(data.stats?.total_revenue) }}</div><div class="stat-label">Revenue</div></div>
        </div>
      </div>
      ...
      <div v-if="data.store?.status === 'approved'" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <RouterLink to="/store/venues" class="card card-body flex items-center gap-4 hover:shadow-md hover:border-accent-200 border border-transparent transition-all group">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">{{ data.stats?.total_venues }} venue{{ data.stats?.total_venues !== 1 ? 's' : '' }} listed</p>
          </div>
        </RouterLink>
        ...
        <RouterLink to="/store/bookings" class="card card-body flex items-center gap-4 hover:shadow-md hover:border-primary-200 border border-transparent transition-all group">
          ...
          <div>
            ...
            <p class="text-xs text-surface-500">{{ data.stats?.pending_bookings }} pending review</p>
          </div>
        </RouterLink>
