  ...
  <div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-3/5 relative flex-col" style="background: linear-gradient(135deg, #eef2ff 0%, #e0e7ff 50%, #ede9fe 100%);">
      ...
      <div class="flex-1 flex flex-col justify-center px-16 pb-16">
        <p class="text-xs font-semibold tracking-widest text-primary-500 uppercase mb-4">Join EventGenius</p>
    ...
    <div class="w-full lg:w-2/5 bg-white flex flex-col items-center justify-center px-8 py-12">
      ...
      <div class="w-full max-w-sm">
        <div class="login-card-theme">
          <div class="mb-6">
            <h2 class="text-xl font-bold text-surface-900">Create your account</h2>
            <p class="text-sm text-surface-500 mt-1.5">
              {{ portal === 'store' ? 'Register as a store owner to list your venues.' : 'Join EventGenius to browse and book venues.' }}
            </p>
          </div>
          ...
          <p class="text-center text-sm text-surface-500 mt-6">
            ...
            <RouterLink :to="loginPath" class="text-primary-600 hover:text-primary-700 font-semibold">Sign in</RouterLink>
          </p>
        </div>
...
const loginPath = computed(() => portal.value === 'store' ? '/store/sign-in' : '/client/sign-in')
...
async function handleRegister() {
  ...
  const payload: Record<string, string> = {
    ...
    role:                  portal.value === 'store' ? 'store_owner' : 'client',
  }
  if (portal.value === 'store') {
