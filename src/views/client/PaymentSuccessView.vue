<template>
  ...
  <div class="p-6 max-w-2xl">
    ...
    <div class="card text-center py-10">
      ...
      <p class="text-gray-500 mb-6">Your payment has been processed via PayFast sandbox.</p>

      <!-- Transaction Details -->
      <div v-if="paymentDetails" class="bg-gray-50 rounded-lg p-4 text-left max-w-sm mx-auto mb-6">
        <h3 class="text-sm font-semibold text-gray-600 mb-3">Transaction Details</h3>
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-gray-500">Booking ID</span>
            <span class="font-mono text-gray-700">#{{ bookingId }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">Payment ID</span>
            <span class="font-mono text-gray-700">{{ mPaymentId }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">Amount</span>
            <span class="font-semibold text-gray-900">PKR {{ Number(paymentDetails.amount).toLocaleString() }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">Status</span>
            <span class="text-green-600 font-medium capitalize">{{ paymentDetails.status }}</span>
          </div>
          <div v-if="paymentDetails.transaction_id" class="flex justify-between">
            <span class="text-gray-500">Transaction</span>
            <span class="font-mono text-gray-700 text-xs">{{ paymentDetails.transaction_id }}</span>
          </div>
          <div v-if="paymentDetails.paid_at" class="flex justify-between">
            <span class="text-gray-500">Paid At</span>
            <span class="text-gray-700">{{ formatDate(paymentDetails.paid_at) }}</span>
          </div>
        </div>
      </div>

      <!-- Confirming state -->
      <div v-else-if="confirming" class="text-gray-400 mb-6">Confirming payment...</div>
