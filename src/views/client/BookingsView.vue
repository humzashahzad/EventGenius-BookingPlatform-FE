  ...
  <div class="client-page">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Track and manage all your venue reservations.</p>
      </div>
    ...
    <div v-if="loading" class="space-y-4">
      <div v-for="i in 4" :key="i" class="client-card h-28 animate-pulse bg-slate-100 rounded-2xl"></div>
    </div>
    ...
    <div v-else class="space-y-4">
      <div
        v-for="b in bookings"
        :key="b.id"
        class="client-card hover:shadow-md hover:border-violet-200 transition-all"
      >
        <div class="client-card-body">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <span class="font-mono text-xs text-slate-400 tracking-wide">{{ b.booking_number }}</span>
              <h3 class="font-semibold text-slate-800 mt-0.5 truncate">{{ b.venue?.name }}</h3>
            ...
            <div class="text-right flex-shrink-0 hidden sm:block">
              <div class="text-lg font-bold text-slate-800">PKR {{ Number(b.total_amount).toLocaleString() }}</div>
              ...
              <RouterLink :to="`/client/bookings/${b.id}`" class="text-xs text-violet-600 hover:text-violet-700 font-medium mt-2 block">View Details →</RouterLink>
            </div>
          </div>
          <div class="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 sm:hidden">
            <span class="font-bold text-slate-800">PKR {{ Number(b.total_amount).toLocaleString() }}</span>
            <span :class="statusClass(b.status)" class="text-xs font-medium px-2.5 py-1 rounded-full capitalize">{{ b.status }}</span>
            <RouterLink :to="`/client/bookings/${b.id}`" class="text-sm text-violet-600 font-medium">View Details →</RouterLink>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="meta.last_page > 1" class="flex items-center justify-center gap-2 pt-6">
      <button @click="changePage(meta.current_page - 1)" :disabled="meta.current_page === 1" class="w-9 h-9 rounded-lg border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 disabled:opacity-50">
      ...
      <div class="flex gap-1">
        <button
          ...
          :class="p === meta.current_page ? 'bg-violet-600 text-white' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'"
        >{{ p }}</button>
      </div>
      <button @click="changePage(meta.current_page + 1)" :disabled="meta.current_page === meta.last_page" class="w-9 h-9 rounded-lg border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 disabled:opacity-50">
