  ...
  >
    ...
    <div class="flex items-center h-14 px-3 border-b border-warm-200 dark:border-warm-700 shrink-0">
      ...
      >
        ...
        <span
          v-show="!collapsed"
          class="font-bold text-warm-800 dark:text-white truncate"
        >
          EventGenius
        </span>
      </RouterLink>
    ...
    <nav class="flex-1 overflow-y-auto scrollbar-thin py-3">
      <p v-show="!collapsed" class="px-3 text-xs font-semibold uppercase tracking-wider text-warm-400 dark:text-warm-500 mb-1">
        Browse
      </p>
      <RouterLink
        to="/venues"
        ...
        :class="{ 'bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 font-medium': isActive('/venues') }"
      >
        <AppIcon icon="map-pin" class="w-5 h-5 shrink-0" />
        <span v-show="!collapsed">Browse Venues</span>
      </RouterLink>

      <p v-show="!collapsed" class="px-3 text-xs font-semibold uppercase tracking-wider text-warm-400 dark:text-warm-500 mt-4 mb-1">
        My Account
      </p>
      <RouterLink
...
const mainNav = computed(() => [
  {
    ...
    label: 'My Bookings',
    icon: 'calendar-event',
    badge: notifStore.bookingUnreadCount > 0 ? notifStore.bookingUnreadCount : undefined,
  },
  {
    to: '/customer/messages',
    label: 'Messages',
    icon: 'message',
    badge: chatStore.totalUnreadCount > 0 ? chatStore.totalUnreadCount : undefined,
  },
  {
